appController.controller(
	'ListerCtrl'
  , ['$scope', '$rootScope', '$http', '$filter', '$timeout', '$sharedFactories', '$shopData', '$cartFactories', 'shoppinglistData', 'History', '$routeParams', '$location'
  , function($scope, $rootScope, $http, $filter, $timeout, $sharedFactories, $shopData, $cartFactories, shoppinglistData, History, $routeParams, $location) {
		var direction
		  , reOffset
		...
		$scope.minicartPath = '/partials/minicart.html';
		$scope.miniwishlistPath = '/partials/miniwishlist.html';
		...
		$scope.detailLink = function (itemId) {
			return angular.element(document.querySelector('html')).hasClass('lte9')
				? '#/detail/' + itemId/* + '/' + item.titel|urlEncode*/
				: '/shop/api/detail/' + itemId/* + '/' + item.titel|urlEncode*/;
		};

		/*$scope.viewImage = function () {
			var e = window.event;
			e.preventDefault();
			pixieBox.viewImage(e);
		};*/
		...
		$scope.filterCategory = function() {
			...
			if ($scope.resyncdata) {
				$scope.currentPage = 1;
				requestData = $shopData.promise({filter : JSON.stringify($scope.categories)});
		...
		$scope.items = shoppinglistData.data.data_set;
		$scope.pageCount = function() {
			return shoppinglistData.data.total_pages;
		};
		...
		$scope.sortBy = function (column) {
			if ($scope.items.length) {
				...
				if (!$scope.resyncdata) {
					...
					var requestData = $shopData.promise({
...
//				window.event.preventDefault();
		...
		$scope.setPage = function(n) {
			...
			if (getNextDataset || getPrevDataset) {
				...
				requestData = $shopData.promise(dataParams);
				requestData.then(function(data) {
					$scope.items = data.data.data_set;
				});
		...
		/*$scope.loadMore = function () {
			...
			if (getNextDataset || getPrevDataset) {
				...
				requestData = $shopData.promise(params);
				requestData.then(function(data) {
					reOffset = 0;
					$scope.items = data.data.data_set; 
					$scope.offset = 0;
